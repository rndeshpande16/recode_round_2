...
const KnowledgeGraph = {
  ...
  render() {
    ...
    const simulation = d3.forceSimulation(nodes)
      .force('link', d3.forceLink(links).id((d) => d.id).distance(120).strength((d) => d.strength * 0.3))
  ...
  renderLegend() {
    ...
    legendList.innerHTML = NexusData.subjects.map((s) => `
  ...
  setupControls() {
    ...
    if (reset) {
      reset.addEventListener('click', () => {
        if (this._svgEl && this._zoom) {
          this._svgEl.transition().duration(500).call(this._zoom.transform, d3.zoomIdentity);
        }
      });
    }
  },
